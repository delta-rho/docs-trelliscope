...
<p><strong>Version:</strong> 0.8.2.4</p>
<p><strong>Date:</strong> 2015-05-04</p>
...
<h4>Usage</h4>
<pre class="r"><code>makeDisplay(data, name, group = "common", desc = "", height = 500,
  ...
  verbose = TRUE, keySig = NULL, params = NULL, packages = NULL,
...
converted to underscores).  Defaults to "common"</dd>
  <dt>desc</dt>
  <dd class="rd-dd">a description of the display (used in the viewer and in notebooks)</dd>
  <dt>height</dt>
...
<dl class="rd-dl">
  ...
  <dt>name</dt>
  <dd class="rd-dd">the name for the VDB (if null, uses the name of the parent directory)</dd>
  <dt>autoYes</dt>
...
<dl class="rd-dl">
  ...
  <dd class="rd-dd">VDB connection info, typically stored in options("vdbConn") at the beginning of a session, and not necessary to specify here if a valid "vdbConn" object exists</dd>
  <dt>port</dt>
  <dd class="rd-dd">what port to use for the viewer</dd>
</dl>

  <h4>Details</h4>

  <p class="rd-p">The state</p>
...
<pre class="r"><code>cogMean(x, desc = "mean", group = "common", defLabel = FALSE,
  defActive = TRUE, filterable = TRUE)</code></pre>
...
<pre class="r"><code>cogRange(x, desc = "range (max - min)", group = "common",
  defLabel = FALSE, defActive = TRUE, filterable = TRUE)</code></pre>
...
Sync localDisk data that is used for VDB displays located throughout the system to a 'data' directory inside the VDB - useful for collecting data before syncing with a web server, and used inside of <code><a href='webSync.html'>webSync</a></code>.
...
<h3>Sync VDB and notebook Files to a Web Server</h3>
...
Airplane data
  ...
  <p class="rd-p">A data frame with 1479 rows and 47 variables</p>
...
Removes whitespace surrounding a trellis plot.  Ideal for <code><a href='makeDisplay.html'>makeDisplay</a></code> because when tiling multiple panels, a lot of space is wasted with the margins.
...
Apply a prepanel function to objects of class "ddo" or "ddf" to determine ranges of x and y axis limits prior to creating a trelliscope display (<code><a href='makeDisplay.html'>makeDisplay</a></code>).  Useful in conjunction with <code><a href='setLims.html'>setLims</a></code>.
...
<h4>Examples</h4>
<pre class="r"><code>irisSplit <- divide(iris, "Species")
