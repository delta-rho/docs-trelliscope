...
Before getting into the details, we'll first go over a quick example to provide a feel for what can be done with Trelliscope.  This example is adapted from the quick start example in the [datadr](http://deltarho.org/docs-datadr) documentation, but with a specific focus on Trelliscope.
...
Our example is based on a small dataset that we can handle in a local R session, and therefore we only need to have these two packages installed.  For support of more scalable back ends like Hadoop when dealing with larger data sets, see the [quickstart section](http://deltarho.org/#quickstart) on the DeltaRho website.
...
An interesting thing to look at with the housing data is the median list and sold price over time by county and state.  To split the data in this way, we use the `divide()` function from the `datadr` package.  It is recommended to have some familiarity with the [datadr](http://deltarho.org/docs-datadr) package.
