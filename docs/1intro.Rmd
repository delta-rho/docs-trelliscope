...
<div class="alert alert-warning">This tutorial is undergoing revision based on recent updates in the Trelliscope package.  The content is still is relevant as-is for the most part, but will be much better soon.</div>

Trelliscope provides a way to visualize large, complex data in great detail from within the R statistical programming environment.  This package operates on data that has been split into subsets using the [datadr](http://github.com/tesseradata/datadr) package.  Data can be local R data objects or can be very large data sets supported by the [RHIPE](http://github.com/tesseradata/RHIPE) backend of `datadr`.

Trelliscope is based on the concept of Divide and Recombine (D&R).
...
References:
   - [tesseradata.org][tesseradata.org]
...
Trelliscope is a part of and depends on `datadr`.  It is recommended to spend some time in the `datadr` [tutorial](http://tesseradata.org/docs-datadr/index.html) to get acquainted with the package.  Mainly focusing on the [division](http://tesseradata.org/docs-datadr/index.html#division) aspects and the [backend options](http://tesseradata.org/docs-datadr/index.html#backend-choices) of the package will suffice, although it is recommended to get well-acquainted with the entire package, as Trelliscope is simply a visualization component in the D&R analysis paradigm.

#### Installation

To install Trelliscope, we must first install `datadr`, which is discussed [here](http://tesseradata.org/docs-datadr/index.html#getting-started).
...
unlink(file.path(tempdir(), "vdbtest"), recursive=TRUE)
```
