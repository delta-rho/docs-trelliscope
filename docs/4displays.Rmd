...
### hidden
conn <- vdbConn("vdb", autoYes = TRUE, name = "deltarhoTutorial")
```
...
We achieve data partitioning through the `datadr` package, the companion package to Trelliscope.  This package implements the Divide & Recombine (D&R) approach to data analysis.  If you have not spent time with `datadr`, we cover enough in this section to scrape by, but we highly recommend that you spend some time with the `datadr` [tutorial](http://deltarho.org/docs-datadr).
